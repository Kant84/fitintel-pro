...
class CredentialsTab(QWidget):
    ...
    def _search_client(self):
        ...
        self.lbl_name.setText(
            f"{c.get('last_name','')} {c.get('first_name','')} {c.get('middle_name','')}".strip())
        self.lbl_phone.setText(str(c.get("phone", "—")))
        self.lbl_id.setText(str(self._current_client_id))
        self.client_info.setTitle(f"👤  {self.lbl_name.text()}")
        self._log(self.log, f"✓ Выбран: {self.lbl_name.text()}")
    ...
    def _load_credentials(self):
        ...
        r = self._req("GET", f"/credentials/rfid/client/{cid}")
    ...
    def _encode_card(self):
        ...
        if isinstance(r2, dict) and "_err" not in r2:
            self._log(self.log, "✅ Носитель очищен (Пустая карта)")
        else:
            self._log(self.log, f"❌ Ошибка: {r2.get('_body', str(r2))}")
    ...
    def _write_credential(self):
        ...
        if isinstance(r, dict) and "_err" not in r:
            self._log(self.log, f"✅ Привязано: {vendor} {model}, UID={uid}")
            ...
            self._log(self.log, f"❌ Ошибка привязки: {msg}")
    ...
    def _reset_card(self):
        ...
        r2 = self._req("POST", "/reader/write", {
            ...
            "block": 3,
            "key_type": "A",
            "data_hex": "ffffffffffffffffffffffff078069ffffffffffff"
        })
        if isinstance(r2, dict) and "_err" not in r2:
            self._log(self.log, "✅ Носитель сброшен (factory default)")
        else:
            self._log(self.log, f"❌ Ошибка сброса: {r2.get('_body', str(r2))}")

    def _check_reader(self):
        try:
            r = self._req("GET", "/reader/status")
            if r.get("connected"):
                self.reader_status.setText(f"🟢  {r.get('reader_name', 'Считыватель подключён')}")
                self.reader_status.setStyleSheet("color: #10b981; font-weight: 600;")
            else:
                self.reader_status.setText("🔴  Считыватель не найден")
                self.reader_status.setStyleSheet("color: #ef4444; font-weight: 600;")
        except Exception as e:
            self.reader_status.setText(f"🔴  {str(e)[:50]}")
            self.reader_status.setStyleSheet("color: #ef4444; font-weight: 600;")

    # ═══════════════════════════════════════════════════════
    #  TAB 2 — Программирование замков
    # ═══════════════════════════════════════════════════════
    def _admin_tab(self):
        w = QWidget()
        ...
        lay.addWidget(QLabel("<h3 style='color:#ec4899;'>🔧  Программирование оффлайн-замков</h3>"))
        ...
        col1.addWidget(make_btn("📍  Сменить сектор", BTN_WARNING, "set_sector"))
        grid.addLayout(col1)
        ...
        col3.addWidget(make_btn("⏰  Таймер открытия", BTN_SECONDARY, "open_time"))
        grid.addLayout(col3)
    ...
    def _admin_cmd(self, cmd):
        ...
        commands = {
            "encode":      ("00000000000000000000000000000000", 1),
            "reset_card":  ("ffffffffffffffffffffffff078069ffffffffffff", 3),
            "set_sector":  (f"SECTOR{sector:02d}".encode().hex().ljust(32, "0")[:32], 1),
            "auth_card":   ("AUTH_CARD_ITC".encode().hex().ljust(32, "0")[:32], 2),
            "admin_card":  ("ADMIN_CARD_ITC".encode().hex().ljust(32, "0")[:32], 2),
            "assign_num":  ((f"LOCKNUM{lock_num.zfill(4)}" if lock_num else "LOCKNUM0000").encode().hex().ljust(32, "0")[:32], 2),
            "individual":  ("INDIVIDUAL_MODE".encode().hex().ljust(32, "0")[:32], 2),
            "remove_mode": ("REMOVE_MODE".encode().hex().ljust(32, "0")[:32], 2),
            "open_time":   ("OPENTIME_0000".encode().hex().ljust(32, "0")[:32], 2),
            "reset_lock":  ("RESET_LOCK".encode().hex().ljust(32, "0")[:32], 2),
        }
        data_hex, block = commands.get(cmd, ("0" * 32, 1))

        log.appendPlainText(f"▶ {cmd}  |  сектор {sector}  |  блок {block}")
        ...
        if isinstance(r2, dict) and "_err" not in r2:
            log.appendPlainText(f"  ✅ Команда записана на карту")
            if cmd == "assign_num" and lock_num:
                self.lock_num.setText(str(int(lock_num) + 1))
        else:
            log.appendPlainText(f"  ❌ Ошибка: {r2.get('_body', str(r2))}")
    ...
    def _create_online_key(self):
        ...
        if isinstance(r, dict) and "_err" not in r:
            self.ol_log.appendPlainText(
                f"✅ Ключ создан: {payload['lock_provider']} / {payload['lock_id']}")
        else:
            self.ol_log.appendPlainText(f"❌ Ошибка: {r.get('_body', str(r))}")
