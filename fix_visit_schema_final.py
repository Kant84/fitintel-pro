# fix_visit_schema_final.py
with open('app/schemas/visit.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Убираем model_validator — делаем client_id опциональным без валидации
...
new_code = '''    # Валидация идентификаторов перенесена в endpoint'''

if old_validator in content:
    content = content.replace(old_validator, new_code)
    with open('app/schemas/visit.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("model_validator убран — валидация в endpoint!")
else:
