...
# Заполняем club_daily_metrics
cur.execute("""
    INSERT INTO club_daily_metrics (date, club_id, visits_count, revenue_total, new_contracts_count)
    SELECT date, club_id,
        MAX(CASE WHEN metric = 'attendance' THEN value::int ELSE 0 END),
        MAX(CASE WHEN metric = 'revenue' THEN value ELSE 0 END),
        MAX(CASE WHEN metric = 'new_clients' THEN value::int ELSE 0 END)
    FROM analytics_daily
    WHERE date = CURRENT_DATE
    GROUP BY date, club_id
...
BEGIN
    ...
    lags AS (
        SELECT 
            ...
            m.visits_count - LAG(m.visits_count, 7) OVER (ORDER BY m.d) as trend
        FROM metrics m
    )
