# test_e8_all.ps1
Write-Host "=== E8 — Visits (Посещения) ==="
...
# Получаем клиента с активным абонементом
...
try {
    ...
    Write-Host "  entry_time: $response.entry_time"
...
=== E8.5 Регистрация выхода (positive) ==="
    ...
    try {
        $response = Invoke-RestMethod -Uri "http://localhost:8001/api/v1/visits/check-out" -Method POST -Headers $headers -Body $body
...
try {
    ...
    Write-Host "OK: Доступ запрещён или посещение не найдено"
...
try {
    ...
    Write-Host "  Всего посещений: $($response.total)"
    Write-Host "  На странице: $($response.items.Count)"
} catch {
    Write-Host "ERROR: $($_.Exception.Message)"
}
