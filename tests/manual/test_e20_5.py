...
# Первое использование — должно пройти
first = requests.post("http://localhost:8001/api/v1/dynamic-qr/verify",
                      headers=headers, json={"qr_payload": qr_payload})
print(f"First verify Status: {first.status_code}")

# Второе использование — должно быть отклонено
resp = requests.post("http://localhost:8001/api/v1/dynamic-qr/verify",
                     headers=headers, json={"qr_payload": qr_payload})
print(f"E20.5 Status: {resp.status_code}")
print(f"E20.5 Response: {resp.text[:500]}")
